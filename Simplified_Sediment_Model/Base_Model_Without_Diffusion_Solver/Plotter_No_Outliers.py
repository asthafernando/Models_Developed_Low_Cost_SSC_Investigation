...
# Set professional thesis-ready formatting (Exact match to original)
plt.rcParams.update({
    'font.family': 'serif',
...
# Create specific subfolder for No Outliers
plots_dir = os.path.join(work_dir, "Plots_No_Outliers")
os.makedirs(plots_dir, exist_ok=True)

try:
    # Load model output data
    model_df = pd.read_csv(os.path.join(work_dir, "Model_Output_No_Diffusion.csv"))
    
    # --- FILTER: DISREGARD THE TWO OUTLIERS (Measured > 0.3) ---
    model_df = model_df[model_df['Target_Measured_Avg_s0_g_L'] < 0.3].copy()
    
    print(f"Successfully loaded {len(model_df)} model observations (Outliers Filtered)")
...
ax1.plot([min_val, max_val], [min_val, max_val], 'k--', linewidth=1.5, alpha=0.8, label='1:1 line', zorder=2)
...
for i, ctype in enumerate(canal_types):
    ...
    ax1.scatter(measured_ssc[mask], modelled_ssc[mask], c=[colors[i]], s=80, alpha=0.8, edgecolors='k', linewidth=0.5, label=f'{ctype}', zorder=3)

slope, intercept, r, p, se = stats.linregress(measured_ssc, modelled_ssc)
reg_line = slope * measured_ssc + intercept
ax1.plot(measured_ssc, reg_line, 'r-', linewidth=2, label=f'Fit: y = {slope:.2f}x + {intercept:.3f}', zorder=4)

ax1.set_xlabel('Measured SSC (g L$^{-1}$)')
ax1.set_ylabel('Modelled SSC (g L$^{-1}$)')
ax1.set_title('Modelled vs Measured SSC (Outliers Removed)', fontweight='bold')
ax1.legend(loc='upper left', frameon=True, fancybox=True, shadow=True)
ax1.grid(True, alpha=0.3)

metrics_text = f'R$^2$ = {r2:.3f}\nRMSE = {rmse:.4f} g L$^{-1}$\nNSE = {nse:.3f}\nPBIAS = {pbias:.2f}%'
ax1.text(0.05, 0.95, metrics_text, transform=ax1.transAxes, verticalalignment='top', fontsize=10, bbox=dict(boxstyle='round,pad=0.4', facecolor='wheat', alpha=0.7))
...
ax2.scatter(mean_vals, diff_vals, c='steelblue', s=80, alpha=0.8, edgecolors='k', linewidth=0.5, zorder=3)
ax2.axhline(mean_diff, color='k', linestyle='-', linewidth=2, label=f'Mean: {mean_diff:.4f}', zorder=2)
ax2.axhline(ci_upper, color='r', linestyle='--', linewidth=1.5, label=f'+1.96 SD: {ci_upper:.4f}', zorder=2)
ax2.axhline(ci_lower, color='r', linestyle='--', linewidth=1.5, label=f'-1.96 SD: {ci_lower:.4f}', zorder=2)

ax2.set_xlabel('Mean SSC (g L$^{-1}$)')
ax2.set_ylabel('Difference (Modelled - Measured) (g L$^{-1}$)')
ax2.set_title('Bland-Altman Analysis (Outliers Removed)', fontweight='bold')
...
ax3.scatter(modelled_ssc, residuals, c='seagreen', s=80, alpha=0.8, edgecolors='k', linewidth=0.5)
...
try:
    sns.regplot(x=modelled_ssc, y=residuals, ax=ax3, scatter=False, lowess=True, color='r', line_kws={'linewidth': 1.5})
except:
    pass
...
ax4.hist(residuals, bins=int(np.sqrt(len(residuals)))+1, color='darkorange', alpha=0.7, edgecolor='k', linewidth=1)
ax4.axvline(x=0, color='k', linestyle='-', linewidth=2, label='Zero')
ax4.axvline(x=np.mean(residuals), color='r', linestyle='--', linewidth=2, label=f'Mean: {np.mean(residuals):.4f}')
...
for tide in ['HT', 'LT']:
    ...
    if mask.sum() > 0:
        ax5.scatter(measured_ssc[mask], modelled_ssc[mask], s=90, alpha=0.8, edgecolors='k', linewidth=0.5, label=f'{tide} (n={mask.sum()})')
...
ax5.set_title('Performance by Tide State (Filtered)', fontweight='bold')
...
for i, day in enumerate(days):
    ...
    ax6.scatter(measured_ssc[mask], modelled_ssc[mask], s=90, alpha=0.8, edgecolors='k', linewidth=0.5, label=f'{day} (n={mask.sum()})')
...
ax6.set_title('Performance by Sampling Day (Filtered)', fontweight='bold')
...
ax7.set_title('Normal Q-Q Plot (Filtered)', fontweight='bold')
...
# --- Summary Statistics Output (Markdown) ---
markdown_content = f"""# Model Performance Summary (No Outliers)
...
results_df.to_csv(os.path.join(work_dir, 'Model_Performance_Results_No_Outliers.csv'), index=False)

print(f"\nAll plots and summary files generated in: {plots_dir}")
print("Detailed filtered results saved to Model_Performance_Results_No_Outliers.csv")